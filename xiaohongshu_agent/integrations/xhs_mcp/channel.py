...
from xiaohongshu_agent.apps.xhs.services.constants import DEFAULT_PUBLISH_TIME, DEFAULT_SORT_BY
...
class XiaohongshuChannel:
    ...
    def _init(self):
        ...
        try:
            ...
            if "result" in resp:
                ...
                logger.info(f"MCP 连接成功: {server_info.get('name')} v{server_info.get('version')}")
            else:
    ...
    def _send_request(self, req: Dict, timeout: float | None = None) -> Dict:
        """发送请求"""
        ...
        try:
            ...
            raise MCPError("请求超时", code="TIMEOUT")
    ...
    def _call(self, method: str, tool_name: str | None = None, tool_args: dict | None = None) -> Dict:
        """调用 MCP 方法"""
    ...
    def publish(
        ...
        args: Dict[str, Any] = {"title": title, "content": content, "images": images}
        if tags:
            args["tags"] = tags
        ...
        try:
            ...
            if "result" in result:
                logger.info("发布成功!")
                return PublishResult(success=True, raw=result)
            return PublishResult(success=False, error="发布失败", raw=result)
        except MCPError as e:
    ...
    def get_login_qrcode(self) -> Dict:
        ...
        logger.info("获取登录二维码...")
        try:
            result = self._call("tools/call", tool_name="get_login_qrcode", tool_args={})
            if "result" in result:
                text = result["result"]["content"][0]["text"]
                logger.info("获取二维码成功")
